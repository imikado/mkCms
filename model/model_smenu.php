<?php
class model_smenu extends abstract_model{
	
	protected $sClassRow='row_smenu';
	
	protected $sTable='smenu';
	...
	public function getSelect(){
		...
		if($tab){
		foreach($tab as $oRow){
			$tSelect[ $oRow->id ]=$oRow->smen_title;
		}
		}
		return $tSelect;
	}
	
	public function getSelectIndexed(){
		$tab=$this->findAll();
		$tSelect=array();
		if($tab){
		foreach($tab as $oRow){
			$tSelect[ $oRow->menu_id ][]=$oRow;
		}
		}
		return $tSelect;
	}
...
class row_smenu extends abstract_row{
	
	protected $sClassModel='model_smenu';
